...
namespace LightSide
{
    ...
    public sealed class UniTextWorldBatcher : MonoBehaviour
    {
        ...
        private static void CleanupOnDomainReload()
        {
            ...
            if (stage != null)
                UnityEditor.EditorApplication.delayCall += () => OnPrefabStageOpened(stage);
        }
        ...
        private static void OnPrefabStageOpened(PrefabStage stage)
        {
            ...
            foreach (var world in root.GetComponentsInChildren<UniTextWorld>(true))
            {
                if (mainInstance != null)
                    mainInstance.slots.Remove(world);
            ...
            RecoverMainSceneComponents(stage.scene);
        }

        private static void RecoverMainSceneComponents(Scene prefabScene)
        {
            ...
            {
                var scene = SceneManager.GetSceneAt(i);
                if (!scene.IsValid() || !scene.isLoaded || scene == prefabScene) continue;

                foreach (var rootGo in scene.GetRootGameObjects())
                foreach (var world in rootGo.GetComponentsInChildren<UniTextWorld>(true))
                {
                    if (!world.isActiveAndEnabled || mainInstance.slots.ContainsKey(world)) continue;
        ...
        {
            EnsureInstance();
            var b = GetTargetBatcher(component);
            if (b.slots.ContainsKey(component)) return;
            b.slots[component] = new ComponentSlot();
        }
        ...
        internal static void Unregister(UniTextWorld component)
        {
            var b = GetTargetBatcher(component);
            if (b == null) return;
            b.slots.Remove(component);
            b.meshUploadNeeded = true;
        }
        ...
        internal static void ClearSlotData(UniTextWorld component)
        {
            var b = GetTargetBatcher(component);
            if (b == null) return;
            if (!b.slots.TryGetValue(component, out var slot)) return;
        ...
        internal static void CopySlotData(UniTextWorld component)
        {
            var b = GetTargetBatcher(component);
            if (b == null) return;
            b.CopySlotDataInternal(component);
        }
